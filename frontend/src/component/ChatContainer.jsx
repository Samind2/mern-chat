...
const ChatContainer = () => {
    const { messages, getMessages, isMessageLoading, selectedUser, subscribeToMessages, unsubscribeFromMessages, isFriend, friendRequestSent, friendRequestReceived } = useChatStore();
    ...
    return (
      ...
      {!isFriend && !friendRequestSent && friendRequestReceived && (
        <div className="p-4 text-center text-green-500">
          You must be friend with this user to send messages.
          <button className="btn btn-sm mt-2">Accent friend request</button>
        </div>
      )}
            <MessageInput />
        </div>
